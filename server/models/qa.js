...
module.exports = {
  getQuestions: (params, callback) => {
    ...
    let getQuestionsQuery4 = `SELECT q.question_id, q.question_body, q.question_date, q.asker_name, q.reported, q.question_helpfulness, NULLIF(json_agg(CASE WHEN a.answer_id IS NOT NULL THEN json_build_object(to_char(coalesce(a.answer_id, 0), 'FM99999999'), json_build_object('id', a.answer_id, 'body', a.body, 'date', a.date, 'answerer_name', a.answerer_name, 'reported', a.reported, 'helpfulness', a.helpfulness, 'photos', (SELECT COALESCE(NULLIF(array_agg(p.url) , '{NULL}') , '{}') FROM photos p INNER JOIN answers aw ON p.answer_id = aw.answer_id WHERE aw.answer_id = a.answer_id) )) ELSE NULL END), '["NULL"]' ) AS answers FROM questions q LEFT JOIN answers a ON q.question_id = a.question_id AND product_id = ($1) GROUP BY q.question_id OFFSET ($2) ROWS FETCH NEXT ($3) ROWS ONLY`;
    pool.query(getQuestionsQuery3, [product_id, offset, count], callback)
  },
